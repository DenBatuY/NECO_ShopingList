package com.batuy.neco_shopinglist.billing
...
class BillingManager(private val activity: AppCompatActivity) {
    ...
    private fun setupBillingClient() {
        bClient = BillingClient.newBuilder(activity).setListener(getPurchasesListener())
    ...
    private fun getItem() {
        ...
        bClient?.querySkuDetailsAsync(skuDetails.build()) { bResult, list -> run {
         if (bResult.responseCode==BillingClient.BillingResponseCode.OK){
             if (list!=null){
                 if (list.isNotEmpty()){
                     val bFlowsParam=BillingFlowParams.newBuilder().setSkuDetails(list[0]).build()
                     bClient?.launchBillingFlow(activity,bFlowsParam)
                 }
             }
         }
        } }
            ...
            if (!purchase.isAcknowledged) {
                ...
                bClient?.acknowledgePurchase(acParams) {
                    if (it.responseCode == BillingClient.BillingResponseCode.OK) {
                      savePref(true)
                        Toast.makeText(activity,"Thanks for shop",Toast.LENGTH_SHORT).show()
                    }
                    else {savePref(false)
                        Toast.makeText(activity,"Failure shop",Toast.LENGTH_SHORT).show()}
                }
            }
        }
    }
